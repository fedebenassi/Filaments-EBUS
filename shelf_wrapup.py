...
if __name__ == '__main__':
    ...
    df = pd.DataFrame(columns = ['region', 'content', 'error'], 
    ...
    for i, box in enumerate(boxes):
        ...
        time_series = xr.open_dataarray(f'outputs/shelf_timeseries/box_{i}.nc') * c_to_chl_ratio

        content, error = compute_export_values_bootstrap(time_series)

        df.loc[i] = pd.Series({'region' : i, 'content' : content, 'error' : error})
                  
    df.to_csv('outputs/shelf_content.csv')
