...
def prepare_single_day_data(delta_chl_day, delta_sst_day):
    ...
    data = pd.DataFrame({"chl" : delta_chl_day.chl.values.flatten(), 
                         "sst" : delta_sst_day.sst.values.flatten()}).dropna()
    return data
...
def find_streamer_cluster(train_data, labels):
    """Finds the Streamer cluster (associated to greatest chlorophyll anomaly)
    Input:
        train_data (pandas.DataFrame) : fitted train dataset 
    ...
    return train_data["labels"].iloc[np.argmax(train_data["chl"])]
